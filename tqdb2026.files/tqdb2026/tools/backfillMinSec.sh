...
CMD="${TQDB_DIR}/tools/csvtzconv.py '${TQDB_FromTZ}' 'local' '${CSV_TZFrom}' > ${CSV_TZTo}"
echo "Ready to run: "$CMD
eval $CMD

CMD="cat ${CSV_TZTo} | ${TQDB_DIR}/tools/Min2Cass.py $CASS_IP $CASS_PORT tqdb1 '${SYM_To}'"
echo "Ready to run: "$CMD
eval $CMD
...
CMD="cat ${CSV_TZTo} | ${TQDB_DIR}/tools/Sec2Cass.py $CASS_IP $CASS_PORT tqdb1 '${SYM_To}'"
echo "Ready to run: "$CMD
eval $CMD
