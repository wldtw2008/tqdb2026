...
def _getTradeDateByMkOMkC(iDate, iTime, iMkO, iMkC):
	d = datetime.date(int(iDate/10000), int((iDate/100)%100), int(iDate%100))
	if iMkO==iMkC:
...
def _updateData(iDate, iTime, dbOpen, dbHigh, dbLow, dbClose, dbVol):
	...
	if iTrgDate == -1:
		...
		return
	dt = datetime.datetime(int(iDate/10000), int((iDate/100)%100), int(iDate%100), int(iTime/10000), int((iTime/100)%100), int(iTime%100))	
