...
int main(int argc, char *argv[]) {
	...
	pszQSym = argv[i++];
        pszQDateBeg = argv[i++];
        if (strchr(pszQDateBeg, ' ') == NULL)//add hh:mm:ss
        {
                char* pszDateWithHHMMSS = new char[64];
                sprintf(pszDateWithHHMMSS, "%s 00:00:00", pszQDateBeg);
                pszQDateBeg = pszDateWithHHMMSS;
        }
        pszQDateEnd = argv[i++];
        if (strchr(pszQDateEnd, ' ') == NULL)//add hh:mm:ss
        {
                char* pszDateWithHHMMSS = new char[64];
                sprintf(pszDateWithHHMMSS, "%s 00:00:00", pszQDateEnd);
                pszQDateEnd = pszDateWithHHMMSS;
        }
