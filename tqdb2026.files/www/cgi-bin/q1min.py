...
import os
import subprocess
import urllib,urllib.parse,json
...
def getFirstValidDateTime(symbol, begDTstr, endDTstr):
    ...
    url = 'http://127.0.0.1/cgi-bin/qSymRefPrc.py?symbol=%s&qType=LastValidPrc&qDatetime=%s' % (urllib.parse.quote(symbol), urllib.parse.quote(begDTstr))
    resp = urllib.request.urlopen(url)
    obj = json.loads(resp.read())
    #print obj
    if obj != None and trgType in obj and obj[trgType][0] != None:
        begRefDT = obj[trgType][0]['datetime']
    url = 'http://127.0.0.1/cgi-bin/qSymRefPrc.py?symbol=%s&qType=LastValidPrc&qDatetime=%s' % (urllib.parse.quote(symbol), urllib.parse.quote(endDTstr))
    resp = urllib.request.urlopen(url)
    obj = json.loads(resp.read())
    #print obj
    if obj != None and trgType in obj and obj[trgType][0] != None:
...
def doCustomSymbol(tmpFile): 
	...
	subprocess.call("python3 ./q1min_multileg.py '%s' '%s' '%s' '%s' '%d'" % (profile,begDT,endDT,tmpFile,iGZip), 
...
def loopReadFromStdin(tmpFile):
	...
	fp = open(tmpFile, 'rb')
	#only buffer.write can output byte
