...
def _procPostData():
    ...
    if (tzConv == 'on' and tzSelect is not None and tzSelect != ''):
        ...
        try:
            with open('/etc/timezone', 'r') as f:
                param['tzFromTo'][1] = f.readline().decode('ascii').strip()
        except:
            pass
    else:
        param['tzFromTo'] = None 
    ...
    if param['tzFromTo'] is not None:
        ...
        runCmd = "cd /tqdb2026/tools ; ./csvtzconv.py '%s' '%s' '/tmp/%s.tzFrom' > /tmp/%s.tzTo" % (param['tzFromTo'][0], param['tzFromTo'][1], importTicket, importTicket)
...
def _prepareImport():
    ...
    with open(cmdfile, 'w') as cmd:
        cmd.write('source /tqdb2026/profile_tqdb.sh\n')
        cmd.write('TQDB="tqdb1"\n')
        sym = param['Sym']
        sym = sym.replace('$', '\\$')
        cmd.write('SYMBOL="%s"\n'% sym)
        cmd.write('NDAYAGO=0\n')
        cmd.write('CMD="cat /tmp/%s.csv | python3 -u $TQDB_DIR/tools/Min2Cass.py $CASS_IP $CASS_PORT $TQDB \'$SYMBOL\'"\n' % importTicket)
